...
import { BRollLibrary } from '@/features/editor/deps/b-roll-library';
...
export const MediaSidebar = memo(function MediaSidebar() {
  ...
  const categories = [
    ...
    { id: 'b-roll' as const, icon: Clapperboard, label: 'B-Roll' },
    ...
    <div className="flex h-full flex-shrink-0">
      ...
      >
        ...
        <Activity mode={leftSidebarOpen ? 'visible' : 'hidden'}>
          ...
          {/* B-roll Tab — Cutflow library (category → subcategory → clips) */}
          <div className={`flex-1 overflow-hidden ${activeTab === 'b-roll' ? 'block' : 'hidden'}`}>
            <BRollLibrary />
          </div>
