...
class MediaLibraryService {
  ...
  /**
   * Import a File (e.g. fetched remote b-roll) into OPFS and the project library.
   * Mirrors handle-based import but stores bytes in OPFS instead of a live handle.
   */
  async importMediaFromFile(
    ...
    const thumbnailSaved = !!thumbnailId;
    ...
    try {
      ...
      if (resolvedMimeType.startsWith('video/')) {
        proxyService.setProxyKey(id, getSharedProxyKey(mediaMetadata));
      }
      ...
      if (metadataCreated) {
        try {
          ...
          logger.warn(`Failed to roll back remote import metadata ${id}:`, cleanupError);
        }
      }
      ...
      if (thumbnailSaved) {
        ...
        try {
          ...
          logger.warn(`Failed to roll back thumbnail ${id}:`, cleanupError);
        }
      }
      ...
      try {
        ...
        logger.warn(`Failed to roll back OPFS file ${opfsPath}:`, cleanupError);
      }
